...
import type { ApplicantMetrics } from "@/lib/api";

// Hiring widgets beside the funnel: four semantic stat tiles + a top-sources card.
// No charts this phase (locked) — tones carry the meaning. Mirrors LeadMetrics.
...
export function HiringMetrics({ metrics }: { metrics: ApplicantMetrics | null }) {
  // In pipeline = the non-terminal (worked) stages: applied, screening, interview, offer.
  const inPipeline =
    metrics?.stages
      .filter(
        (s) =>
  ...
  return (
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
      ...
      <Tile
        ...
        value={metrics?.avg_days_to_hire != null ? String(metrics.avg_days_to_hire) : "—"}
        icon={Timer}
        tone="warning"
      />
