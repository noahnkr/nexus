...
- `[-]` Task 1 — Config + `wh_client.py` (export CSV pager w/ Link cursors + rate respect, JSON reference endpoints) + offline fixtures; gated live ping test *(config + client written; fixtures + tests pending)*
- `[ ]` Task 2 — Migration `20260730000000_entities_crm_sync.sql` (leads `zip`/`address`/`background`, `lead_contacts` mirroring `client_contacts` + RLS) + seam threading; gated schema tests
...
- `[ ]` Task 5 — `views/leads.change_stage` extraction (single stage-writer; REST + tool delegate) + WH mapping (`wh_map.py`) + resolution update path (`updates_entity`, `UPDATERS`) + Start-of-Care client promotion (`clients.lead_id`, contacts copy, `client.created`) + verbatim `leads.source` (M16 contract); offline + gated tests
- `[ ]` Task 6 — WH runner + `backfill_welcomehome.py` (idempotent, resumable, promotion-bounded + discharge-sweep runbook) + `ingest_text` transcription ingestion via the M15 document entity tag; offline runner tests + **live backfill against the real account**
- `[ ]` Task 7 — Wrap-up: README/.env.example; full pytest; **live incremental walk** (WH stage change → lead status + `lead.stage_changed` + sequence fires within one poll; Start-of-Care → client on `/clients` + `/referrals` conversion)
