...
- Every inbound webhook/connector event must resolve to a canonical entity via `external_ids` before writing anywhere else — never let a connector write directly into a business table without entity resolution. Events carry `resolve_by` to say which resolution domain they belong to: `"id"` (the default — the source's record id, looked up scoped to an entity type) or `"phone"`, where the entity type is unknown by nature and the number decides it. Phone-keyed events look up `external_ids` **untyped**, then fall through to the seam's `resolve_by_phone` (`entity_writers.py`: match on the last ten digits across people entities and their contact rows — never a raw string compare, since numbers are stored however they were typed; contact rows resolve to their parent; one hit registers the mapping so later events short-circuit; ambiguity or no match → review task, never a guess). Core resolution stays business-agnostic — it calls the seam hook, it never names a vertical table
- All inbound connector traffic enters through the single ingest seam `services/connectors/ingest.py::ingest_payload` (raw receipt written to `events` before normalization) — the `POST /api/webhooks/{source}` route is verify-then-ingest, poll/export sources are driven by the in-app connector sync loop (`services/connectors/sync.py`, a lifespan loop under `get_machine_tenant_id()` like the automations engine loops) whose runners fetch → translate → call the same `ingest_payload`, and push sources that neither poll nor post use a **bridge** — a long-lived socket held open by its own lifespan task that decodes frames and calls the same seam. A bridge is transport only: no verification, no resolution, no writes of its own, and never a second inbound path. Provider channels expire, so channel upkeep belongs to a sync-loop runner (create-when-absent / replace-before-expiry / no-op-when-healthy) while the bridge just reconnects when the channel changes. Every lifespan task must stay cancellable — cancellation propagates through every await, or shutdown hangs. Sync cursors/channel state live in `connector_state.state`; credentials and OAuth refresh tokens live in env vars only, never in the database. Connector adapters live in `backend/app/services/connectors/adapters/` (one file per source: `verify` + `normalize` only); `services/connectors/entity_writers.py` is part of the vertical re-templating seam alongside the entity migration and `services/tools/entities.py`. External platforms are source of truth: sync is one-way inbound; outbound effects go through gated tools only (no entity write-back)
...
- `entity_summaries` is keyed by `(tenant_id, entity_type, entity_id, kind)` — `smart_summary` is the one kind in use (the single per-entity summary covers record, activity, and communication history; its comms context comes from recency reads over the communications tier, never a second cached artifact); the summary seam (`services/views/summary.py`) threads `kind` through its cache for future derived kinds. Tone/style/responsiveness is a summary problem, never retrieval
