...
**⚠️ Awaiting routing — one defect left from the first live Google/GoTo run (2026-07-22).** Full reproduction detail is in `PROGRESS.md` → _First live Google/GoTo run_. *(Three others — the Calendar `syncToken`, GoTo call subscriptions, and GoTo SMS subscriptions — were fixed and live-verified the same day.)*
