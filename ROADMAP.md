...
- **Orphaned WelcomeHome events & communications on the dev DB** — during the v1.1.2 build, `test_wh_runner`'s cleanup (then matching `external_id like 'wh:%'`) deleted the 90 live-synced leads and their contacts/clients. The Task 6 re-sweep re-imported them from WelcomeHome, but with **new entity ids**, so roughly 8k `events` rows and 276 `communications` still reference the deleted ones and no longer join to anything. Both tables are immutable/derived by design, so the choice is prune-or-leave, not repair. The test cleanup itself is already fixed (narrowed to the fixture's own ids). Dev-DB hygiene only — no production impact.
