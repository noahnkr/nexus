...
|---|---|---|---|
| v1.1.4 | One smart summary per entity | *(to plan)* | **One "at a glance" section per entity**, covering the record, activity, communication history, and significant facts — the separate communication-profile card is folded in, so leads and clients lose the second card and its `comm-profile` endpoints and caregivers gain comms coverage in the summary they already have. The summary draws on the entity's correspondence through retrieval over the communications tier, not a second cached artifact. Seam-local (`services/views/summary.py`) — the `entity_summaries` `kind` column stays. Buildable now: it depends on the comms *tier*, not on which connector fills it. **Expect thin comms coverage until v1.2.0/v1.3.0 land** — the structure is right either way. Retires the *Summary freshness* backlog item's two-cache half (one cache, one Regenerate). |
| v1.2.0 | GoTo Connect | `v1.2.0-goto-connect.md` | **The authoritative source for calls + SMS.** WebSocket bridge, real `send_sms`, call transcripts into the communications tier and thus into RAG. One-time OAuth consent ops step (self-service — no third party in the loop). **Plan must include a known-numbers guard** so WelcomeHome's provisional bridge number doesn't ingest as real client calls: a WH-initiated call dials the office first, so GoTo logs a leg to/from that number which is plumbing, not correspondence. |
