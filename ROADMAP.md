...
- **Call transcripts — recording IS enabled on this account; only the fetch path is missing.** *Corrected 2026-07-22 by live capture, superseding the earlier "no recordings exist" conclusion.* That conclusion came from a recording **search** endpoint that rejected every query grammar, and read a broken search as absent data. Two real calls captured today each carry `recordings: [{transcriptEnabled: true}]` and a transcript id, and `GET /recording/v1/recordings/{id}` answers `200 {"status":"UPLOADED"}`. What is still unknown is how to fetch the transcript **text** — `/recording/v1/recordings/{id}/transcript`, `/recording/v1/transcripts/{id}` and `.../media` all 404. Finding that path makes calls join RAG the way texts already do, via `ingest_communication`. **This also raises a question v1.2.0 answered wrongly:** the CHANGELOG tells the reader calls carry no transcript because the account has no recording, and that sentence is now false. *Separate lead: voicemail transcription endpoints (`/voicemail/v1/voicemails/search`, `.../transcriptions`) answer 403 `AUTHZ_INSUFFICIENT_SCOPE` — reachable by adding a voicemail scope and re-consenting. Voicemail ≠ calls.*
...
- **DONE 2026-07-22 — the frame shape is verified, and it found a blocker.** Real frames from two live calls are captured in `tests/fixtures/goto/` and pinned by `test_goto_live_frames.py`. The envelope differs from the docs (`data.type` is `"call-state"`; `ENDING` is nested at `content.state.type`), and critically **`participants` is empty on every call with no phone number anywhere**, so a call cannot be resolved to a person. Fixing that is a v1.2.x patch and needs a routing decision — a post-frame detail lookup via `conversationSpaceId`, or `REPORT_SUMMARY` under a re-consented `cr.v1.read` scope. See `PROGRESS.md` for the full capture.
