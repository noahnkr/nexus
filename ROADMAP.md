...
**⚠️ Awaiting routing — three defects from the first live Google/GoTo run (2026-07-22).** These are confirmed bugs in shipped code, not ideas, so they need version slots rather than a backlog line each. Full reproduction detail is in `PROGRESS.md` → _First live Google/GoTo run_. *(A fourth — the Calendar runner never storing a `syncToken` — was fixed in v1.3.0 the same day and live-verified.)* In rough severity order:
