...
**⚠️ Awaiting routing — four defects from the first live Google/GoTo run (2026-07-22).** These are confirmed bugs in shipped or in-flight code, not ideas, so they need version slots rather than a backlog line each. Full reproduction detail is in `PROGRESS.md` → _First live Google/GoTo run_. In rough severity order:

- **The Calendar runner never stores a `syncToken` on a real calendar** — a 10-page × 250-result cap is hit before the last page, and `nextSyncToken` only arrives on the last page, so every cycle re-sweeps ~2,500 events and incremental sync never begins. Blocks v1.3.0 from shipping; the fix (a `timeMax` bound and/or a higher cap) belongs *in* v1.3.0 rather than after it, since the version is not yet documented as shipped.
- **GoTo call subscriptions 400 on every cycle** (`"The Notification Channel must be a WebSocket channel"`) — v1.2.0's phone channel does not work in production despite shipping green. Two candidate causes already probed live and disproven; cause still unknown. Patch version against v1.2.0.
- **GoTo inbound SMS never subscribes and fails silently** (`ownerPhoneNumber` rejected, then swallowed into a zero-id "success"). Same patch as above; the silent-success path is the part worth fixing regardless of the number format.
- **v1.1.5's Windows startup fix is inert** — uvicorn builds its event loop before importing the app, so the policy set in `main.py` applies to nothing. The documented command works only because it carries `--reload`. Needs an entrypoint script; until then the `--reload` flag is load-bearing and should not be described as optional.
...
- **Verify the GoTo notification frame shape against a live call** — v1.2.0's `gt_map` fixtures for the call-history record are real (captured from the account), but the WebSocket envelope comes from GoTo's published Call Events Report schema rather than an observed frame. `gt_map` scans structurally for numbers rather than trusting field names, so a shape drift degrades to "no counterpart found → ack-only receipt" rather than bad data on a record — but the first live call is the real test, and the fixture should be replaced with a captured frame once one exists.
